...
const FeatureCard = ({ icon, title, description, color, bg }: Feature) => {
  return (
    <div
    ...
    >
      <div className="relative z-10 bg-white p-6 rounded-xl h-full">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 ${bg}`}>
          {React.cloneElement(icon, {
            className: `w-6 h-6 ${color.replace("from-", "text-").split(" ")[0]}`,
          })}
        </div>
      ...
      <div
        className={`absolute -bottom-10 -right-10 w-32 h-32 rounded-full bg-gradient-to-br ${color} opacity-0 group-hover:opacity-10 transition-opacity duration-500`}
      />
    </div>
  );
};
