...
import ABI from "../../../smart_contract/src/ABI.json" assert { type: "json" };
...
export async function signMessage(message: string): Promise<{ address: string; signature: string }>{
...
export async function donateEth({ contractAddress, campaignId, valueEth }: DonateParams): Promise<string> {
  const provider = await ensureProvider();
  const signer = await provider.getSigner();
  const contract = new Contract(contractAddress, ABI as any, signer);
  const tx = await contract.donate(BigInt(campaignId), { value: parseEther(valueEth) });
...
export async function switchOrAddChain(target: { chainIdHex: string; rpcUrls?: string[]; chainName?: string; nativeCurrency?: { name: string; symbol: string; decimals: number }; blockExplorerUrls?: string[] }) {
  const eth = getEthereum();
  if (!eth) throw new Error("MetaMask not detected.");
  try {
    await eth.request({ method: "wallet_switchEthereumChain", params: [{ chainId: target.chainIdHex }] });
  } catch (err: any) {
    if (err?.code === 4902 && target.rpcUrls && target.chainName && target.nativeCurrency) {
      await eth.request({
